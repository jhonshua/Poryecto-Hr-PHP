...
#== 2. Obteniendo datos del archivo .XML =========================================
    ...
    foreach ($params as $param) {
        ...
        if ($param->getAttribute('Impuesto')=="002"){ // IVA
            $ImporteTotalIVArete  = $ImporteTotalIVArete + $param->getAttribute('Importe');
            $ImporteTotalIVArete = $param->getAttribute('Importe');
        }
    }
		//var_dump($ImporteTotalIVArete);exit();
...
#== Registros de artículos ================================================
    ...
    if ($Puntero<=23.5){
        SubTotales($pdf, $Y, $subTotal, $descuento, $ImporteTotalIVA, $ImporteTotalIEPS,$ImporteTotalIVArete, $total, $formaDePago, $metodoDePago, $NumCtaPago, $Puntero);
    }
...
function SubTotales($pdf, $Y, $subTotal, $descuento, $ImporteTotalIVA, $ImporteTotalIEPS,$ImporteTotalIVArete, $total, $formaDePago, $metodoDePago, $NumCtaPago, $Puntero){
   ...
   if($ImporteTotalIVArete>0){
    $pdf->SetFont('arial','B',9);
    $pdf->SetXY($X+16.27,$Y+0.35+(0.5*4));
    $pdf->Cell(1.7, 0.30, "Impuestos Retenidos:", 0, 0,'R', 0);

    $pdf->SetFont('arial','',9);
    $pdf->SetXY($X+16.16+2.5,$Y+0.35+(0.5*4));
    $pdf->Cell(1.7, 0.30, number_format($ImporteTotalIVArete,2), 0, 1,'R', 0);

    }
    $pdf->SetFont('arial','B',9);
    $pdf->SetXY($X+16.27,$Y+0.37+(0.5*5));
    $pdf->Cell(1.7, 0.30, "Total:", 0, 1,'R', 0);    

    $pdf->SetFont('arial','',9); 
    $pdf->SetXY($X+16.16+2.5,$Y+0.37+(0.5*5));

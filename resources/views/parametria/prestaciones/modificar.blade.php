...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="container">
            ...
            <label class="font-size-1-5em mb-5 under-line font-weight-bold">Parametría / Tipo de prestaciones / Modificar tipo de prestación </label>
            ...
            <div class="article border general-div" id="general-div" >
                <form action="{{route('parametria.prestaciones.modificarRegistros',['id'=>$data['id']])}}" method="POST" id="form" >
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                        <label>Nombre de la prestación:</label>
                          <input type="text" class="form-control input-style-custom" name="nombre" value="{{$data['nombre']}}" placeholder="Ingrese un nombre para la prestación" >
                          {!! $errors->first('nombre','<p class="text-danger text-center mt-3">Error: El campo nombre es requerido</p>') !!} 
                        </div>
                        <div class="col-md-6">
                            <label>Tipo clase prestación:</label>
                            @php $id_clase = Crypt::decrypt($data['tipo_clase']);@endphp
                            <select name="tipo_clase" class=" form-control  select-clase" style="width:100% !important"  >
                                <option value="" disabled selected >Seleccione una clase</option>
                                @foreach ($clases as $clase)
                                    <option value="{{$clase->id}}" {{($clase->id ==$id_clase ) ? 'selected' : ''}} >{{$clase->tipo_clase}}</option>
                                @endforeach
                            </select>

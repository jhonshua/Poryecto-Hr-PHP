...
<div class="container">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th class="border-warning">Timbre</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <!-- <div class="col-12 text-center">
        <form method="POST" action="{{route('timbrar.aguinaldo.paso1')}}">
            @csrf
            <input type="hidden" name="id_periodo" value="{{$periodo}}" >
            <input type="hidden" value="1" name="todos" id="todos">
            <input type="hidden" value="{{ $cadena_departamentos }}" name="deptos" id="deptos">
        ...
        methods: {
            async T(cadena) {
                const promises = this.empleados.map(async (emp, idx) => {
                    let url = urlBase + '/timbrar-aguinaldo-empleado/' + emp.id + '/' + btoa(cadena) + '/2';
                    $("#sp" + emp.id).html(sp);

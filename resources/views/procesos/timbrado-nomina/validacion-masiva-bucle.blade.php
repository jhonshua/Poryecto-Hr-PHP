    ...
    <div class="article border">
        ...
        <div class="row justify-content-center">
            <div class="col-xs-12 col-md-12 col-lg-3">
                <a href="{{route('timbrar.nomina.emailMasivo', [$periodo, base64_encode($cadena_departamentos), $regresa])}}"
                   class="button-style btn-block mb-3">Enviar Avisos</a>
            </div>
        </div>
...
<script>
    ...
    let urlBase = "{{ url('/') }}";
    let msg_exito = '<span class="badge badge-success">Proceso Exitoso</span>';
                    ...
                    <div id="acc1ordion">
                        <div class="card">
                            ...
                            <div id="collapse_${id}" class="collapse" aria-labelledby="headingOne" data-parent="#accordion">
                                <div class="card-body">
                                    <textarea rows="6" cols="100" readonly="yes">
                                    ${xml}
                                    </textarea>
                                </div>
                            </div>
        ...
        methods: {
            async T(cadena) {
                const promises = this.empleados.map(async (emp, idx) => {
                    let url = urlBase + '/timbrar/nomina/empleado/' + emp.id + '/' + cadena + '/2/' + this.regresa;
                    $("#sp" + emp.id).html(sp);
                    await axios.get(url).then(response => {
                        if (response.data.exito) {
                            $("#sp" + emp.id).html(badgeOk);
                            $("#sp_mensaje" + emp.id).html(msg_exito);
                        } else {
                            $("#sp" + emp.id).html(badgeError);
                            $("#sp_mensaje" + emp.id).html(soap_mensaje(emp.id,response.data.data.codigo_error,response.data.data.MENSAJE_error,response.data.data.archivo_xml));
                            this.errors++;
                        }
                    }).catch(error => {
                        $("#sp" + emp.id).html(badgeError);

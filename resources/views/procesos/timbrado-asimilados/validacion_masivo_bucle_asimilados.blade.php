...
@section('content')
<div class="row mt-5" id="appTimbrar">
    ...
    <div class="col-12 text-center">
      ...
      <a name="" v-show="ok" id="" class="btn btn-dark" href="#" role="button">Enviar avisos</a>
    </div>
    ...
    methods: {
      async T(cadena) {
        const promises = this.empleados.map(async (emp, idx) => {
          let url = urlBase + '/contabilidad/timbrar/asimilados/empleado/'+emp.id+'/'+cadena+'/2';
          $("#sp"+emp.id).html(sp);
          await axios.get(url).then(response => {
                console.log(response);

                if(response.data.exito ){
                    $("#sp"+emp.id).html(badgeOk);
                }else{
                    $("#sp"+emp.id).html(badgeError);
                    errors++;
                }                
            }).catch(error => {
              $("#sp"+emp.id).html(badgeError);
        ...
        alertify.success('Timbrado Correcto');
        console.log('Finished!');
},
      timbrar: function(cadena) {

        this.empleados.forEach( function(valor, indice) {
          var url = urlBase + '/contabilidad/timbrar/asimilados/empleado/'+valor.id+'/'+cadena+'/2';
          $("#sp"+valor.id).html(sp);
          console.log(url);
          axios.get(url).then(response => {
                ...
                */
                ...
                if(response.data.exito ){
                    $("#sp"+valor.id).html(this.badgeOk);
                    $("#sp"+valor.id).html(this.badgeOk);
                }else{
                    $("#sp"+valor.id).html(this.badgeError);
                }

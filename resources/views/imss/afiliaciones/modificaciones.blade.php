...
<div class="container">
	@include('includes.header',['title'=>'Movimientos afiliatorios - Modificaciones', 'subtitle'=>'IMSS', 'img'=>'img/header/administracion/icono-usuario.png', 'route'=>'afiliaciones.inicio'])


	<div class="row">
		<div class="col-md-12 d-flex mt-4">
	        <a href="{{route('afiliaciones.inicio')}}" class="btn button-style ml-3">Altas</a>
	        <a href="{{route('afiliaciones.bajas')}}" class="btn button-style ml-3">Bajas</a>
	        {{-- <a href="{{route('imss.afiliaciones.modificaciones')}}" class="btn btn-warning mr-3">Modificaciones</a> --}}

	        <a href="#" class="btn button-style text-nowrap ml-3 disco">Generar disco (Modiificaciones)</a> | 


	        <span class="text-nowrap pt-2 mr-2 ml-3">Búsqueda por:</span>
	                ...
	                <tbody>
	                    @foreach ($empleados as $empleado)
	                        ...
	                        >
	                            ...
	                            <td width="140px">
	                                <input type="text" name="folio_modificacion" id="folio_modificacion" class="form-control folio_alta" value="{{$empleado->folio_modificacion}}">
	                            </td>

	                            <td width="170px" class="position-relative">
	                                
	                                @if (empty($empleado->folio_modificacion))
	                                    <button class="btn btn-warning btn-sm guardar" data-id_empleado="{{$empleado->id}}">Guardar</button>
	                                @elseif ($empleado->estatus_folio_modificacion  != 2)
	                                    <button class="btn btn-warning btn-sm guardar" data-id_empleado="{{$empleado->id}}">Guardar</button>
...
<script type="text/javascript">
    ...
    $('.btn.guardar').click(function(){
        ...
        folio_modificacion = btn.parents('tr').find('#folio_modificacion').val();
        
        var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        data = {
            'id' : id,
            'folio_modificacion': folio_modificacion,
            '_token': CSRF_TOKEN
        }

        var url = "{{route('afiliaciones.guardarFolioModificacion')}}";
    ...
    $('.btn.cerrar').click(function(){
        ...
        var url = "{{route('afiliaciones.cierreFolioModificacion')}}";
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.ok == 1) {
                    ...
                    btn.parents('tr').find('#folio_modificacion').attr('disabled', true);
                } else {
    ...
    $('.btn.disco').click(function(){
		...
		.then((willDelete) => {
		  	if (willDelete) {
			    ...
			    window.location.href = "{{route('afiliaciones.generarDiscoModificaciones')}}?f=" + folio;
		  	} else {

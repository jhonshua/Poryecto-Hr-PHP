...
$archs_cfdi = "../resources/timbrado/archs_cfdi/";
$archs_backup_qr = "../resources/timbrado/bck_qr/";

$url = "../public/repositorio/".$repo."/timbrado/";
...
$NomArchPDF='PDF_Asimilados_Masivo_Periodo_'.$id_periodo.'.pdf';
...
while ( $resultTim = mysqli_fetch_array( $rowqueryTim ) ) {
    ...
    $queryemisora="SELECT upper(ememi.razon_social) AS razon,ememi.user_timbre,ememi.cp AS cp,em.rfc AS rfcemi,regpat.num_registro_patronal 
                         ...
                         JOIN $base.categorias cat on em.id_categoria_asimilados = cat.id
     ...
     $queryvalorisr = "SELECT ROUND(isr_asimilados, 2) 
    ...
    $ArrayDeduc_Importe       = array();
       ...
       $ArrayDeduc_Clave         = array();
            ...
            $idconce           = 'ISR';
            $idTipoconceptoDed = '46';
            $idconcededuc      = '46';
            $str02             = $idTipoconceptoDed;
            $validaidDed       = strlen($str02);
            if( $validaidDed < 3 ){
               $idTipoconceptoDed = str_pad('46', 3, "DD", STR_PAD_LEFT); 
            }else{
                $idTipoconceptoDed = 'DD'.'46';
            }
           ...
           $queryvalorimportededuci = "SELECT  ROUND(isr_asimilados, 2) AS result 
                                       FROM rutinas$ejercicio
                                       WHERE id_periodo = '$id_periodo' 
                                       AND id_empleado = '$id_empleado' 
                                       AND fnq_valor = 0;";
    ...
    $repoUser = "../public/repositorio/".$repo. "/". $id_empleado ."/timbrado/archs_qr";

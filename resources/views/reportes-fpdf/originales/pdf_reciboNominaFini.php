...
include '../php/numerosletras.php';

    $idempleado=$_GET['id'];
    

//fechaAlta,idempleado,numsegurosocial,departamento,curp,categoria
    $querydirectorio="SELECT *,substring_index(categoriaesp,' ',2) as puesto,substring_index(departamento,' ',2) as depa from $base.empleado where idempleado='$idempleado'";
    $rowresultdirec=$db->command($querydirectorio);
    $repositorio=$_SESSION['dir'].$rowresultdirec['repositorio'];
    

    $queryemisora="SELECT upper(emi.RazonSocial) as razon from singh.asigna_empresasemisoras asi join singh.empresas e on asi.idEmpresa=e.idEmpresa inner join 
singh.empresasemisoras emi on asi.ID_EmpresaE=emi.ID_EmpresaE where e.RazonSocial='$Empresa' and asi.status<>2;";

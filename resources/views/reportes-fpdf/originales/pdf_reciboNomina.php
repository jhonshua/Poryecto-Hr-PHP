...
$idempleado = $_GET['id'];
$bd_id = intval(str_ireplace('Empresa000','',$base));

//fechaAlta,idempleado,numsegurosocial,departamento,curp,categoria
$querydirectorio = "SELECT *, substring_index(categoria_esp,' ',2) as puesto,substring_index(departamento,' ',2) as depa,
concat(nombre,' ',apaterno,' ',amaterno) as name from $base.empleados where id ='$idempleado'";
...
$archs_cfdi = '../public/repositorio/'.$bd_id.'/'.$idempleado;
$archs_graf = '../public/repositorio/'.$bd_id.'/'.$idempleado;
...
if (isset($_GET['is'])) {
    $valorisr = base64_decode($_GET['is']);
} else {
...
$Nomina_FechaInicioLaboral = $rowresultdirec['fecha_alta'];
$Nomina_NoEmpleado         = $rowresultdirec['id'];
$Nomina_NoImss             = $rowresultdirec['num_seguro_social'];
...
foreach ($params as $param) {
    if (strlen($param->getAttribute('Nombre')) > 0) {
        ...
        $Receptor_Nom = $rowresultdirec['name'];
...
$pdf->image("../public/repositorio/FondoTenue.jpg", $X + 1, $Y + 8.6, 19.5, 18);
...
$pdfFinal = $SendaArchsCFDI . $NomArchPDF;
// $pdf->Output($pdfFinal, 'F'); -- Se guarda a un arcivo
$pdf->Output($NomArchPDF, 'D'); //  -- se descarga
...
// chmod($pdfFinal, 0777); // <-- Descomentar si está utilizando el sistema operativo LINUX.
// $pdf->Output($pdfFinal, 'I'); // Se muestra el documento .PDF en el navegador.

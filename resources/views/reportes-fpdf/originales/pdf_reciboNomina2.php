<?php
	...
	$idempleado=$_GET['id'];    
    $querybase="SELECT Base from empresas where RazonSocial='$Empresa'";
    $rowbase=mysqli_query($con,$querybase);
    $rowresultbase=mysqli_fetch_array($rowbase);
    $base=$rowresultbase['Base'];
    ...
    $rowquerydirectorio=mysqli_query($con,$querydirectorio);
    $rowresultdirec=mysqli_fetch_array($rowquerydirectorio);
    $repositorio=$rowresultdirec['repositorio'];
    

    $queryemisora="SELECT upper(emi.RazonSocial) as razon from asigna_empresasemisoras asi join empresas e on asi.idEmpresa=e.idEmpresa inner join
empresasemisoras emi on asi.ID_EmpresaE=emi.ID_EmpresaE where e.RazonSocial='$Empresa' and asi.status<>2;";
$rowemisora=mysqli_query($con,$queryemisora);
$rowresultemisora=mysqli_fetch_array($rowemisora);
...
// =============================================================================
    ...
    $pdf->SetXY($X+1,$Y+20.32);
    $pdf->Cell(3.7, 0.25, utf8_decode("Recibí de conformidad la cantidad descrita, haciendo constar que con dicha suma estoy totalmente pagado (a) hasta la fecha"), 0, 1,'L', 0);
    $pdf->SetXY($X+1,$Y+20.72);
    $pdf->Cell(3.7, 0.25, utf8_decode("señalada en el presente recibo, por lo que no me reservo acción ni derecho alguno para reclamar por estos conceptos ni por"), 0, 1,'L', 0);

    $pdf->SetXY($X+1,$Y+21.10);
    $pdf->Cell(3.7, 0.25, utf8_decode("ningún otro."), 0, 1,'L', 0);
...
// <-- Descomentar si está utilizando el sistema operativo LINUX.
 // Se muestra el documento .PDF en el navegador.

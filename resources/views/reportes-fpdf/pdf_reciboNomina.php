...
$url = ".../public/storage/repositorio/".$db_id."/".$id_empleado."/timbrado";
$folder_qr = $url . '/archs_qr/';
$folder_pdf = $url . '/archs_cfdi/';
...
$querydirectorio = "SELECT *,concat(nombre,' ',apaterno,' ',amaterno) as name from empleados where id ='$id_empleado'";
$empleado = $db->command($querydirectorio);

$querydirectorio = "SELECT * from puestos where id ='" . $empleado['id_puesto'] . "'";
$puesto = $db->command($querydirectorio);

$query = "SELECT * from departamentos where id ='" . $empleado['id_departamento'] . "'";
$departamento = $db->command($query);

// $queryemisora = "SELECT upper(ememi.razon_social) as razon, user_timbre, ememi.cp as cp from $base.empleados em join $base.categorias cat on em.categoria=cat.nombre inner join registro_patronal regpat on cat.tipo_clase=regpat.id inner join empresas_emisoras ememi on regpat.id_empresa_emisora=ememi.id where id='$idempleado' and  ememi.estatus=1 and cat.estatus=1 and ememi.estatus=1;";
...
$PaginaWeb        = "https://hrsystem.com.mx";
...
$Nomina_FechaInicioLaboral = $empleado['fecha_alta'];
$Nomina_NoEmpleado         = $empleado['id'];
$Nomina_NoImss             = $empleado['nss'];
$Nomina_Departamento       = $departamento['nombre'];
$Nomina_CURP               = $empleado['curp'];
$Nomina_Puesto             = $puesto['puesto'];
...
} else if ($param->getAttribute('TotalDeducciones')) {
...
foreach ($params as $param) {
    ...
    $ArrayPercep_ImporteGravado[$i] = $param->getAttribute('ImporteGravado') + $param->getAttribute('ImporteExento');
...
// $pdf->Cell(1.5, 0.25, utf8_decode('"'.$emisora.'"'), 0, 1,'L', 0);
//echo $emisora;
/*if($emisora=='DESARROLLADORA DE EMPRESAS ACT SA DE CV'){

$rutaImagen=$archs_graf."/Emi_Desa.jpg";

}else if($emisora=='BRAIN MASTER S. DE R.L. DE C.V.'){

$rutaImagen=$archs_graf."/Emi_Brain.jpg";

}else if($emisora='REINGENIERÃ­A Y SOLUCIONES ADMINISTRATIVAS CONTABLES, S.C.'){

$rutaImagen=$archs_graf."/Emi_Reing.jpg";

}

$pdf->image($rutaImagen,$X+1, $Y+1 , 9, 2.3);
...
$pdf->Cell(1.5, 0.25, $Concept_Descuento !== '' ? number_format($Concept_Descuento, 2) : '', 0, 1, 'R', 0);
...
/*$pdf->SetXY($X+13.3,$Y+11.32+(0.55*12));
$pdf->Cell(3.7, 0.25, utf8_decode("Otros pagos:"), 0, 1,'L', 0);

$pdf->SetXY($X+13.3,$Y+11.32+(0.55*13));
$pdf->Cell(3.7, 0.25, utf8_decode("Neto recibido:"), 0, 1,'L', 0);*/
...
$pdf->Cell(2, 0.25, number_format($Nomina_NumDiasPagados), 0, 1, 'R', 0);

$pdf->SetXY($X + 18.3, $Y + 11.32 + (0.55 * 8));
$pdf->Cell(2, 0.25, number_format($Nomina_TotalPercepciones), 0, 1, 'R', 0);

if ($Nomina_TotalDeducciones) {
...
/*$pdf->SetXY($X+18.3,$Y+11.32+(0.55*12));
$pdf->Cell(2, 0.25, $Nomina_TotalOtrosPagos, 0, 1,'R', 0);

$pdf->SetXY($X+18.3,$Y+11.32+(0.55*13));
$pdf->Cell(2, 0.25, number_format($total,2), 0, 1,'R', 0);*/

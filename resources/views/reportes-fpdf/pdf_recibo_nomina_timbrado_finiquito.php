...
$nombre_xml  = base64_decode($_GET["nombre_archivo"]);
...
$anio_ejercicio = $_GET["anio_ejercicio"];

$nombre_pdf = str_replace('.xml', '.pdf', $nombre_xml);

$archs_graf = "../../../storage/app/public/timbrado/archs_graf/";
$url = "../../../storage/app/public/repositorio/".$id_empresa."/".$id_empleado."/timbrado";
$url_xml = $url . '/archs_cfdi/' . $nombre_xml;
$url_pdf = $url . '/archs_pdf/' . $nombre_pdf;
...
$query_empleado = "select *, concat(e.nombre,' ',e.apaterno,' ',e.amaterno) nombre_completo,
                    (select nombre from ".$empresa.".departamentos where id=e.id_departamento) departamento,
                    (select puesto from ".$empresa.".puestos where id=e.id_puesto) puesto
                  FROM ".$empresa.".empleados e WHERE id = ".$id_empleado;
$datos_empleado = $db->command($query_empleado);

/*$query_concepto_nomina = "SELECT id from ".$empresa.".conceptos_nomina
     where tipo=1 and finiquito=1 and estatus<>0 and file_rool<>0 and file_rool<251
     and rutinas='ISR'";
$datos_concepto_nomina = $db->command($query_concepto_nomina);
$id_isr=$datos_concepto_nomina['id'];

$query_total_rutinas = "SELECT  ROUND(total".$id_isr.", 2) as total_rutinas from ".$empresa.".rutinas".$anio_ejercicio." where id_periodo=".$id_periodo." and id_empleado=".$id_empleado." and fnq_valor=1 "; 
$datos_total_rutinas = $db->command($query_total_rutinas);
$importe_valor_isr = $datos_total_rutinas['total_rutinas'];*/
...
$Nomina_FechaInicioLaboral=$datos_empleado['fecha_alta'];
$Nomina_NoEmpleado        = $datos_empleado['id'];
$Nomina_NoImss            = $datos_empleado['nss'];
$Nomina_Departamento      = $datos_empleado['departamento'];
$Nomina_CURP              = $datos_empleado['curp'];
$Nomina_Puesto            = $datos_empleado['puesto'];
...
$filename = $archs_graf."/Img_".$UUID.".png";
...
for ($i=0; $i<$TotRegs; $i++){
    ...
    $pdf->Cell(1.7, 0.35, number_format($ArrayPercep_ImporteGravado[$i]+$ArrayPercep_ImporteExento[$i],2), 0, 1,'R', 0);

    $SumaPercep = $SumaPercep + $ArrayPercep_ImporteGravado[$i]+$ArrayPercep_ImporteExento[$i];
...
if($Nomina_TotalDeducciones>0){
	...
	// print_r($ArrayDeduc_TipoDeduccion); exit;
	...
	for ($i=0; $i<$TotRegs; $i++){
	    ...
	    $pdf->SetXY($X+2.4+1.3,$Y);
	    $pdf->MultiCell(7, 0.35, utf8_decode(utf8_decode($ArrayDeduc_Concepto[$i])), 0, 'L', 0);
	    $pdf->Write(0.4,'');
	    $YY = $pdf->GetY()+0.18;
...
// echo $id_empleado, '--', $nombre_xml, '--', $id_empresa, '--', $empresa;
print_r($anio_ejercicio);

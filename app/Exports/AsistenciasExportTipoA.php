...
class AsistenciasExportTipoA implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    public function collection()
    {
        ...
        $empleados = Empleado::where('estatus', [Empleado::EMPLEADO_ACTIVO])
         ...
         $asistencias = AsistenciaHorario::select('id_empleado', 'dia', 'entrada', 'salida', 'comida', 'inicio_comida', 'fin_comida', 'asistencia', 'retardo', 'permiso', 'lugar','motivo')
        ...
        $this->dias_festivos = HorariosDias::select('fecha_festiva')
        ...
        foreach ($empleados as $empleado) {
            foreach ($fechas_a_mostrar as $fecha1) {
                ...
                if ( array_key_exists($empleado->id . '_' . $fecha, $asistencias2) ) {
                    $entrada = ($asistencias2[$empleado->id.'_'.$fecha]->permiso == 1) ? 'NO APLICA': (( $asistencias2[$empleado->id.'_'.$fecha]->entrada !="" && $asistencias2[$empleado->id.'_'.$fecha]->entrada !=NULL)?date('H:i:s', strtotime($asistencias2[$empleado->id.'_'.$fecha]->entrada)):"SIN REGISTRO");
                    $salida = ($asistencias2[$empleado->id.'_'.$fecha]->permiso == 1 )? 'NO APLICA': (( $asistencias2[$empleado->id.'_'.$fecha]->salida !="" && $asistencias2[$empleado->id.'_'.$fecha]->salida !=NULL && $asistencias2[$empleado->id.'_'.$fecha]->salida != $asistencias2[$empleado->id.'_'.$fecha]->entrada)?date('H:i:s', strtotime($asistencias2[$empleado->id.'_'.$fecha]->salida)):"SIN REGISTRO");
                    ...
                    if( $asistencias2[$empleado->id.'_'.$fecha]->retardo == 1 || $asistencias2[$empleado->id.'_'.$fecha]->asistencia == 0){
                        $permiso = ( $asistencias2[$empleado->id.'_'.$fecha]->permiso == 1 ) ? (explode("$", $asistencias2[$empleado->id.'_'.$fecha]->motivo))[1] : 'NO';

                    }

                    if( $asistencias2[$empleado->id.'_'.$fecha]->comida == 1 ){
                        $inicio_comida = ( $asistencias2[$empleado->id.'_'.$fecha]->inicio_comida !="" && $asistencias2[$empleado->id.'_'.$fecha]->inicio_comida !=NULL)?date('H:i:s', strtotime($asistencias2[$empleado->id.'_'.$fecha]->inicio_comida) ):"---";
                    ...
                    if ( $asistencias2[$empleado->id.'_'.$fecha]->asistencia == 0 ){ 
                         $this->inasistencias[] = $renglon; }else{ $this->noinasistencias[] = $renglon; 
                    }

                    if ( $asistencias2[$empleado->id.'_'.$fecha]->retardo == 1 ){ 
                        $this->retardos[] = $renglon; }else{ $this->noretardos[] = $renglon;
                     }

                    if ( $asistencias2[$empleado->id.'_'.$fecha]->lugar == 'APP' ){ 
                        $this->nohomeoffice[] = $renglon; }else{ $this->homeoffice[] = $renglon; 
                    }
                }elseif($this->esFechaFestiva($empleado->id_horario, $fecha)){
                    $entrada = 'DÍA FERIADO O INHABIL';
                    $salida = 'DÍA FERIADO O INHABIL';
                }elseif($dias_laborables[$num_dia] == 0 ) {
    ...
    protected function esFechaFestiva($id_horario, $fecha){
        return ($this->dias_festivos->where('id_horario')->where('fecha_festiva', $fecha)->count() > 0 ) ? true : false ;
    }
